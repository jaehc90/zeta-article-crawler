...
count_checker = "(시|명|가지|살|마리|포기|송이|수|톨|통|점|개|벌|척|채|다발|그루|자루|줄|켤레|그릇|잔|마디|상자|사람|곡|병|판|건)"
...
def normalize_number(text):
    ...
    text = normalize_unit(text, unit_to_kor2)
...
def normalize_path(pathname, verbose=False):
    # import pdb; pdb.set_trace()
    import glob
    files = glob.glob(pathname)
    for f in files:
        import os
        outf = os.path.splitext(f)[0]
        outf = outf + ".cleaned"
...
if __name__ == "__main__":
    ...
    parser.add_argument('-path', help='Input File Path (overrides other options)')
    parser.add_argument('-i', help='Input file')
    parser.add_argument('-o', help='Output file')
    parser.add_argument('-v', help='verbose')
    # parser.add_argument('-t', help='Testing normalizer')
    args = parser.parse_args()

    if args.path is not None:
        normalize_path(args.path, args.v)
        ...
        print("please --help for further detail")
    else:
        txt = ""
        with open(args.i, 'r' ) as file:
            txt = file.read()
        text = normalize(txt)
        print(text) if args.v else {}
